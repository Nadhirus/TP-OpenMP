...
int main(int argc, char **argv)
{
	...
	time_t t;
    time(&t); // Obtenir le timestamp actuel
    char corner_image[50];
    sprintf(corner_image, "output_images/corners_%ld.pgm", t);

    // Écrire l'image de sortie
    write_imagef(corner_image, original_image, width, height);
    printf("Image de sortie écrite dans le fichier : %s\n", corner_image);
...
unsigned int find_features(data_wrapper_t *eigenvalues, int max_features, int image_width, int image_height, data_wrapper_t **features)
{
	...
	qsort(eigenvalues, image_size, sizeof(data_wrapper_t), sort_data_wrapper_value_desc);

	// Create the features buffer based on the max_features value.
	*features = (data_wrapper_t *)malloc(sizeof(data_wrapper_t) * max_features);

	unsigned int features_count = 0;
	const int ignore_x = 3;	 // ignore this many pixel rows from top/bottom of image
	const int ignore_y = 3;	 // ignore this many pixel columns from left/right of image
	int stop_processing = 0; // flag to indicate when to stop

#pragma omp parallel for shared(features_count, stop_processing)
	for (int i = 0; i < image_size; ++i)
	{
		if (stop_processing)
			continue; // if flag is set, skip further processing

		// Ignore edges of the image.
...
#pragma omp critical
			{
				if (features_count < max_features)
				{
					...
					if (features_count >= max_features)
					{
						stop_processing = 1; // set flag to stop further processing
					}
				}
			}
		}
...
#pragma omp parallel for collapse(2) private(ixx_sum, iyy_sum, ixiy_sum, i, j, k, m, offsetj, offseti)
	// la directive collapse(2) combine les 2 premieres boucles i et j en une seule grande boucle déroulées et parallelisé
	// les boucles internes k et m ne sont pas parallélisée car elles dépendent du calcul des pixels i et j
	// private pour mettre chaque variable de chaque thread private
...
#pragma omp parallel for collapse(2) private(sum, i, j, k, m, offsetj, offseti)
	// la directive collapse(2) combine les 2 premieres boucles i et j en une seule grande boucle déroulées et parallelisé
	// les boucles internes k et m ne sont pas parallélisée car elles dépendent du calcul des pixels i et j
	// private pour mettre chaque variable de chaque thread private
...
void gen_kernel(float *gkernel, float *dkernel, float sigma, int a, int w)
{
	...
	sum_gkern = 0;
	sum_dkern = 0;
#pragma omp parallel for reduction(+ : sum_gkern, sum_dkern)
	for (i = 0; i < w; i++)
	{
